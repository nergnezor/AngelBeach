	...
	const TSharedPtr<FJsonObject>& PropsJson)
{
	...
	{
		FProperty* Prop = ScriptStruct->FindPropertyByName(FName(*FString(Pair.Key.ToView())));
		if (!Prop)
		{
			Failures.Add(FString::Printf(TEXT("%s: property not found on %s"), *FString(Pair.Key.ToView()), *ScriptStruct->GetName()));
			continue;
		}
		...
		if (!Result)
		{
			Failures.Add(FString::Printf(TEXT("%s: ImportText failed for value '%s'"), *FString(Pair.Key.ToView()), *ValueStr));
		}
	}
